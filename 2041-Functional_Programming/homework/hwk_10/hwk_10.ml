(*Code - With Inspirations frm RealWorldOcaml*)
...
module Complex_arithmetic : (Arithmetic_intf with type t = (float * float)) = struct
  ...
  let mul (a,bi) (c,di) = (a *. c +. -1.0 *. (bi *. di) , a *. di +. c *. bi) 
end 
...
module Make_vector(Arith_mod : Arithmetic_intf) : 
  ...
  let create (size:int) (initVal:t):vector= 
    let rec create_list (size:int) (initVal:t) =  if size = 0 then [] 
...
let cmplxmul = Complex_vector.scalar_mul (5.0, 5.0) v5
let scmplx = Complex_vector.to_string cmplxmul
